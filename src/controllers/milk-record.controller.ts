...
import { insertMilkRecordSchema, milkRecordsTable } from '../db/schemas/milk-record.schema.js';
 ...
 */
export const recordMilkDelivery = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const responseObject = await db.transaction(async (trx) => {
        ...
        const farmerCheck = await trx.select({ id: farmersTable.id })
            .from(farmersTable)
            .where(
                and(
        ...
        if (farmerCheck.length === 0) {
            throw new AppError(ERROR_CODES.NOT_FOUND, `Farmer with ID ${farmer_id} not found or does not belong to this collection center.`);
        }

        // Insert milk record
        const newRecords = await trx.insert(milkRecordsTable)
            .values({
                farmer_id,
        ...
        if (newRecords.length === 0) {
            throw new AppError(ERROR_CODES.INTERNAL_ERROR, 'Failed to record milk delivery.');
        }
    ...
    return ApiResponse.created(res, responseObject, 'Milk recorded successfully.');
});